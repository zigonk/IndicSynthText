...
This will download a data file to the `SynthTextGen` directory. This data file includes:
  ...
  - **data/newsgroup**: Text-source text cropus for the indic languages. This can be subsituted with any text file. Look inside `text_utils.py` to see how the text inside this file is used by the renderer.
...
python gen.py --viz
```
This script will generate random scene-text image samples and store them in an lmdb file in output path specified. If the `--viz` option is specified, the generated output will be visualized as the script is being run; omit the `--viz` option to turn-off the visualizations. If you want to visualize later, run:
